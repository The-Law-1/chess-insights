...
const analysisProgressValue = computed(() => {
	...
	return Math.round((analysisProgress.value.gamesDone / analysisProgress.value.gamesTotal) * 100);
});
	...
	<div class="dashboard">
		...
		<main class="main-content">
			...
			<div
				...
				:class="{ collapsed: !showControls && hasAnalysedGames }">
				<div
					...
					<div class="controls-title">
						...
						<h3
							v-else
							class="page-subtitle">
							Analyse your chess.com games with Stockfish
						</h3>
					</div>
					<button
						v-if="hasAnalysedGames"
				...
				<div
					...
					<form
						...
						<p
							v-if="stockfishFailed"
							class="stockfish-warning">
					...
					<!-- Progress -->
					<div
						v-if="progress.monthsTotal > 0 || analysisProgress.gamesTotal > 0"
						class="progress-area">
						...
						<div class="progress-row">
							...
							<div class="progress-bar">
								<div
									...
									:style="{ width: analysisProgressValue + '%' }"></div>
							</div>
							<span
								v-if="analysisProgress.currentLabel"
								class="progress-meta">
					...
					<p
						v-if="analysisStatus === 'error' || stockfishFailed"
						class="error-msg">
						{{ analysisError }}
					</p>
					<p
				...
				<div
					class="large-set-warning-container"
					v-if="maxGames > 350">
					<div class="warning-text">Large game sets may take a while to load.</div>
					<!-- only display this warning on mobile -->
					<div class="warning-text mobile-warning">
						Analysis of large game sets may be slow on mobile devices. Try on desktop for better
						performance.
					</div>
				</div>
			</div>
			...
			<div
				...
				<p class="empty-desc">
					Your games will be fetched and analysed with Stockfish to reveal patterns in your play.
				</p>
			</div>
		</main>
	</div>
