...
const buildTimeProfile = (frames: ReturnType<typeof parseGame>["frames"], playerSide: "w" | "b"): TimeProfile => {
	...
	const openingTimes = withTime.filter((frame) => frame.moveNumber <= 10).map((frame) => frame.timeSpentSeconds ?? 0);
	const middlegameTimes = withTime
		.filter((frame) => frame.moveNumber >= 11 && frame.moveNumber <= 30)
		.map((frame) => frame.timeSpentSeconds ?? 0);
	const endgameTimes = withTime.filter((frame) => frame.moveNumber >= 31).map((frame) => frame.timeSpentSeconds ?? 0);
...
export const analyseGame = async (
	...
	const totalMoves = parsed.frames.length ? parsed.frames[parsed.frames.length - 1].moveNumber : 0;
	...
	return {
		...
		playerCastledAtMove:
			playerSide === "w"
				? (parsed.metadata.whiteCastledAtMove ?? null)
				: (parsed.metadata.blackCastledAtMove ?? null),
