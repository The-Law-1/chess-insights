...
const ALWAYS_REASONS = new Set<KeyReason>(["major-piece-capture", "promotion", "check-after-capture", "pre-checkmate"]);

const OPTIONAL_PRIMARY_REASONS = new Set<KeyReason>(["minor-piece-capture", "large-clock-drop", "castling"]);
...
export const extractKeyPositions = (frames: MoveFrame[], options: KeyPositionOptions = {}): KeyPosition[] => {
	...
	frames.forEach((frame, index) => {
		...
		const sameSquareCapture =
			frame.isCapture && previousCaptureSquare !== null && currentCaptureSquare === previousCaptureSquare;

		const filteredReasons = sameSquareCapture ? reasons.filter((reason) => !CAPTURE_REASONS.has(reason)) : reasons;
