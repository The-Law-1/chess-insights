...
const buildEndgameType = (chess: Chess): EndgameType | null => {
	...
	const nonPawnTotal =
		counts.w.q + counts.w.b + counts.w.n + counts.w.r + counts.b.q + counts.b.b + counts.b.n + counts.b.r;
	if (nonPawnTotal > 4) {
		return null;
	}

	const buildMaterial = (count: typeof counts.w) =>
		`K${"Q".repeat(count.q)}${"B".repeat(count.b)}${"N".repeat(count.n)}${"R".repeat(
			count.r
		)}${"P".repeat(count.p)}`;

	const base = `${buildMaterial(counts.w)}${buildMaterial(counts.b)}`;
	const bishopTotal = counts.w.b + counts.b.b;
	const bishopSignature = bishopTotal >= 2 ? `${counts.w.bl}${counts.w.bd}${counts.b.bl}${counts.b.bd}` : null;
...
const normalizeSan = (san: string) =>
	san.replace(/0-0-0/g, "O-O-O").replace(/0-0/g, "O-O").replace(/[+#]/g, "").replace(/[!?]+/g, "").trim();
