...
const gamesWithoutData = computed(() => props.games.filter((g) => allMinorDevelopedAt(g) === null).length);
				...
				tooltip: {
					callbacks: {
						...
						label: (ctx: any) => {
							...
							return [`Win rate: ${ctx.parsed.y.toFixed(1)}%`, `${count} game${count !== 1 ? "s" : ""}`];
						}
					}
				}
			}
...
<template>
	<div class="piece-activity">
		...
		<p
			v-else
			class="empty-state">
			No piece activity data available.
		</p>
	</div>
</template>
