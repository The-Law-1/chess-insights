...
const totalGames = computed(() => props.games.filter((g) => g.totalMoves > 0).length);
	...
	<div class="game-length-chart">
		...
		<p class="description">
			...
			<span v-if="totalGames > 0">
				{{ totalGames }} game{{ totalGames !== 1 ? "s" : "" }} analysed, bucketed by {{ BUCKET_SIZE }}-move
				intervals (min 3 games shown).
			</span>
		</p>

		<div
			v-if="chartData.maxBucket > 0"
			class="chart-wrapper">
