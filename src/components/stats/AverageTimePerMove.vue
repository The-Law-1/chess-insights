...
const gamesWithoutData = computed(
	() =>
		props.games.filter((g) => !g.timeProfile.averageSecondsPerMove || g.timeProfile.averageSecondsPerMove <= 0)
			.length
);

const gamesWithData = computed(() => props.games.length - gamesWithoutData.value);
	...
	<div class="avg-time-chart">
		...
		<p class="description">
			...
			<span v-if="gamesWithData > 0">
				{{ gamesWithData }} game{{ gamesWithData !== 1 ? "s" : "" }} with data, bucketed by
				{{ BUCKET_SIZE }}-second intervals (min 3 games shown)
				<template v-if="gamesWithoutData > 0"> ({{ gamesWithoutData }} excluded — no time data). </template>
			</span>
		</p>

		<div
			v-if="chartData.maxSec > 0"
			class="chart-wrapper">
