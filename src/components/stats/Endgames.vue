...
const endgameTypes = computed(() => {
	...
	return Object.values(endgameMap)
		.map((endgame) => ({ ...endgame, winrate: endgame.count > 0 ? (endgame.winrate / endgame.count) * 100 : 0 }))
		.sort((a, b) => b.count - a.count);
});

const rows = computed(() =>
	endgameTypes.value.map((endgame) => ({
