	...
	return function (Stockfish) {
		...
		if (E) {
			...
			(D
				...
				0 !== G.indexOf("blob:") ? (G = G.substr(0, G.lastIndexOf("/") + 1)) : (G = ""),
				E
					? ((H = function (a, b) {
							J || (J = require("fs"));
							K || (K = require("path"));
		...
		if (F) ((k = z.wasmMemory), (l = z.buffer));
			...
			((k = new WebAssembly.Memory({ initial: Ba / 65536, maximum: 32768, shared: !0 })),
		...
		var A = {
			...
			Ea: function () {
				...
				A.fa = [];
				for (a = 0; a < A.ia.length; ++a) ((c = A.ia[a]), (b = c.ea), A.ya(b), c.terminate());
				A.ia = [];
			},
			ya: function (a) {
				if (a) {
		...
		var Vb = {
			...
			n: function (a, b, c) {
				...
				var e;
				for (c >>= 2; (e = t()[b++]); )
					((e = 105 > e) && c & 1 && c++, lb.push(e ? fa()[c++ >> 1] : v()[c]), ++c);
			...
			k: function (a, b, c, e) {
				if ("undefined" === typeof SharedArrayBuffer)
					return (
						N("Current environment does not support SharedArrayBuffer, pthreads are not available!"),
						6
					);
		...
		var Ta = (z._emscripten_register_main_browser_thread_id = function () {
			...
			Tb = (z._emscripten_sync_run_in_main_thread_4 = function () {
				return (Tb = z._emscripten_sync_run_in_main_thread_4 = z.asm.T).apply(null, arguments);
			}),
					...
					if (Zb) {
						...
						try {
							...
							u instanceof L ||
								("unwind" == u
									? (noExitRuntime = !0)
									: ((c = u) && "object" === typeof u && u.stack && (c = [u, u.stack]),
		...
		function Za(a, b) {
			if (!b || !noExitRuntime || 0 !== a) {
				if (!b && F) throw (postMessage({ cmd: "exitProcess", returnCode: a }), new L(a));
				if (!noExitRuntime) {
					A.Ea();
